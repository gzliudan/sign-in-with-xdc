...
app.use(
  Session({
    name: "sign-in-with-xdcpay",
    secret: "xinfin-and-apothem",
    resave: true,
...
app.post("/verify", async function (req, res) {
  try {
    ...
    const msg = `Sign in OK with XDCPay account: ${message.address}`;
...
app.get("/info", function (req, res) {
  ...
  res.setHeader("Content-Type", "text/plain");
  const msg = `You are authenticated with XDCPay account: ${req.session.siwe.address}`;
  res.send(msg);
  console.log(`\n${msg} !`);
});
